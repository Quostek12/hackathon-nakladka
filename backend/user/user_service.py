...
def create_user(user_create: user.UserPost, session: session.SessionDep) -> user.User:
    user_check = session.query(user.User).filter(user.User.login == user_create.login).first() 
    if user_check:
        raise HTTPException(status_code=403, detail="User of this login already exists")
